...
const talks = fs.readFileSync('talks.json', 'utf8');
...
html = html.replace('<script src="script.js"></script>', `<script>${finalJs}</script>`);
html = html.replace(`fetch('/talks.json')
...
fs.writeFileSync('public/index_standalone.html', html);

console.log('Successfully compiled to public/index_standalone.html');
